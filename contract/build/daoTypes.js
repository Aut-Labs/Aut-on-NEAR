...
let DAOTypes = (_dec = NearBindgen({
  requireInit: true
}), _dec2 = initialize({}), _dec3 = view({}), _dec4 = call({}), _dec(_class = (_class2 = class DAOTypes {
  typeToMembershipChecker = new UnorderedMap('type');
  isMembershipChecker = new UnorderedMap('is_m_c');
  owner = '';
  types = 1;

  init() {
    log(predecessorAccountId());
  ...
  addNewMembershipChecker({
    ...
    log("Executing addNewMembershipChecker...");
    log(this.owner, predecessorAccountId());
    log(this.owner === predecessorAccountId());
    assert(this.owner == predecessorAccountId(), 'only owner');
    assert(membershipChecker.length > 0, "MembershipChecker contract address must be provided");
    assert(!this.isMembershipChecker[membershipChecker], "MembershipChecker already added");
    this.typeToMembershipChecker[this.types] = membershipChecker;
    this.isMembershipChecker[membershipChecker] = true;
    this.types++; // emit DAOTypeAdded(_types.current(), membershipChecker);
  }
...
}, (_applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getMembershipCheckerAddress", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "getMembershipCheckerAddress"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "addNewMembershipChecker", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "addNewMembershipChecker"), _class2.prototype)), _class2)) || _class);
function addNewMembershipChecker() {
  let _state = DAOTypes._getState();

  if (!_state && DAOTypes._requireInit()) {
    throw new Error("Contract must be initialized");
  }

  let _contract = DAOTypes._create();

  if (_state) {
    DAOTypes._reconstruct(_contract, _state);
  }

  let _args = DAOTypes._getArgs();

  let _result = _contract.addNewMembershipChecker(_args);

  DAOTypes._saveToStorage(_contract);

  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DAOTypes._serialize(_result));
}
function getMembershipCheckerAddress() {
  let _state = DAOTypes._getState();
  ...
  let _result = _contract.getMembershipCheckerAddress(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DAOTypes._serialize(_result));
}
function init() {
  let _state = DAOTypes._getState();
...
export { addNewMembershipChecker, getMembershipCheckerAddress, init };
//# sourceMappingURL=daoTypes.js.map
