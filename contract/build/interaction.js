...
let Interaction = (_dec = NearBindgen({
  requireInit: true
}), _dec2 = initialize({}), _dec3 = call({}), _dec4 = view({}), _dec5 = view({}), _dec(_class = (_class2 = class Interaction {
  // event InteractionIndexIncreased(address member, uint256 total);
  idCounter = 0;
  daoExpander = '';
  interactions = new UnorderedMap('its');
  interactionsIndex = new UnorderedMap('ii');

  init() {
    this.daoExpander = predecessorAccountId();
    this.idCounter = 0;
  }
  ...
  addInteraction({
    ...
    assert(predecessorAccountId() == this.daoExpander, 'only dao expander');
    let model = {
      member,
      taskID: activityID,
      contractAddress: predecessorAccountId()
    };
    this.idCounter++;
    this.interactions[this.idCounter] = model;
    this.interactionsIndex[member]++; // emit InteractionIndexIncreased(member, interactionsIndex[member]);
  } // view
  ...
  getInteractionsIndexPerAddress({
    user
  }) {
    return this.interactionsIndex[user];
  }

}, (_applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "addInteraction", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "addInteraction"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getInteraction", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "getInteraction"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getInteractionsIndexPerAddress", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "getInteractionsIndexPerAddress"), _class2.prototype)), _class2)) || _class);
function getInteractionsIndexPerAddress() {
  let _state = Interaction._getState();

  if (!_state && Interaction._requireInit()) {
    throw new Error("Contract must be initialized");
  }

  let _contract = Interaction._create();

  if (_state) {
    Interaction._reconstruct(_contract, _state);
  }

  let _args = Interaction._getArgs();

  let _result = _contract.getInteractionsIndexPerAddress(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Interaction._serialize(_result));
}
function getInteraction() {
  ...
  let _result = _contract.getInteraction(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Interaction._serialize(_result));
}
function addInteraction() {
  ...
  let _result = _contract.addInteraction(_args);

  Interaction._saveToStorage(_contract);
...
export { addInteraction, getInteraction, getInteractionsIndexPerAddress, init };
